<template>
    <div class="header-bar" ref="headerBar">
        <div class="header-bar-container">
            <div class="header-bar-section-placeholder"> <!-- Nova div de preenchimento -->
                <img src="/img/rxelms_logo.png" alt="Logo Rxelms"> <!-- Adicione a imagem aqui -->
            </div>
            <div class="header-bar-section">

            <!-- Adicionado o botão Home -->
            <NavBarButton name="Home" to="/" :active="$router.currentRoute.value.fullPath === '/'">
                <h4>Home</h4>
            </NavBarButton>

            <NavBarButton name="Marketplace" to="/marketplace" :active="$router.currentRoute.value.fullPath === '/marketplace'">
                <h4>Marketplace</h4>
            </NavBarButton>

            <NavBarButton name="Documents" to="/documents" :active="$router.currentRoute.value.fullPath === '/documents'">
                <h4>Documents</h4>
            </NavBarButton>

            <NavBarButton name="Builder" to="/builder" :active="matchExclude('/artists', 'docs')">
                <h4>Builder</h4>
            </NavBarButton>

            <NavBarButton name="Rxelms" to="/Rxelms" :active="$router.currentRoute.value.fullPath.includes('/Rxelms')">
                <h4>rxelms</h4>
            </NavBarButton>
            ...
            <div class="header-bar-section-dif">
                    <Tooltip :on="!wide" v-show="!accountStore.activeWallet?.address" class="nav-button-different">
                        <NavBarButton name="Login" @click="login()"
                    ...
                    <div class="header-bar-section-dif-login">
                        <Tooltip :on="!wide" v-show="accountStore.activeWallet?.address">
                            <DisplaysNavbarAvatar :detailed="wide" :name="name"
                                :address="accountStore.activeWallet?.address || ''" :url="accountStore.userInfo?.user.artist?.page.profileImage ?
                                    resolveCid(accountStore.userInfo.user.artist.page.profileImage, 128) :
                                    ((NFDDetails && NFDDetails.v && NFDDetails.v.avatar) ? nfdImage : '')
                                    " @click="login()" @logout="login()" />
                        </Tooltip>
                    </div>
...
.header-bar-container {
    ...
    margin-left: 200px;
    display: flex;
    justify-content: space-between; // Mantém o conteúdo espaçado entre os lados
