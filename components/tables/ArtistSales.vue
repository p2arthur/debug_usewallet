  ...
  <TablesBaseExpand>
    ...
    <template v-slot:rows>
      <template v-for="(sale, index) in sales" :key="index">
        <tr>
          <td class="artist-sales-table-avatar">
            <router-link :to="`/nfts/${sale.nftIndex}/activity`" class="artist-sales-table-avatar-media">
          ...
          <td class="artist-sales-table-time">
...
import type { Sale, GetSalesByArtistQuery } from '@/graphql/client'
import { openAddress } from '@/functions/Explorer'
import { resolveCid } from '@/functions/media'

defineProps({
  sales: {
    type: Array as PropType<Array<Sale> | GetSalesByArtistQuery['getSalesByArtist']['sales'] >,
    required: true
  }
})
