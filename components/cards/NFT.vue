  ...
  <div class="nft-card">
    <div
      ...
      <div class="nft-card-media-overlay">
        <div
          class="nft-card-media-overlay-nsfw"
          v-show="nft?.info.nsfw && !settings.nsfw && !nsfwOverwrite">
          <button
            class="nft-card-media-overlay-nsfw-button"
            @click="nsfwOverwrite = true">
            <span>Show sensitive content</span>
          </button>
        </div>

        <div
          class="nft-card-media-overlay-ai"
          v-show="nft?.info.ai">
          <span>AI</span>
        </div>
        ...
        <div
          class="nft-card-media-overlay-play"
          v-show="
            (nft.media?.mimeType === 'video/mp4' || type === 'video/mp4') &&
            isOutside
          ">
    ...
    <div class="nft-card-info">
      <router-link
        :to="`/artists/${nft.artist.normalizedName}`"
        class="nft-card-info-avatar">
        <img
          v-if="artistCID"
          :src="artistCID"
          alt=""
          crossorigin="" />
        <span v-else>{{
          nft?.artist.address ? nft.artist.address.slice(0, 2) : 0
        }}</span>
      </router-link>
      ...
      <div
        ...
        v-else-if="
          nft.statistics &&
          nft.statistics.latestSale &&
          nft.statistics.latestSale.price &&
          nft.statistics.latestSale.currency
        ">
        <span
          >last sale:
          {{ numberFormater.format(nft.statistics.latestSale.price) }}</span
        >
        <IconsCurrencyByTag
          :tag="nft.statistics.latestSale.currency"
          style="heigth: 16px; width: 16px" />
      </div>
    </div>
  </div>
